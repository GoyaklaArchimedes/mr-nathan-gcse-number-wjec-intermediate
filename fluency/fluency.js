...
(function () {
  ...
  function genTables(format) {
    ...
    // Inverse check (C) is not meaningful for multiplication alone; keep as direct recall variant
    // We still label it C for mix consistency but present as direct.
    return {
      id: `T:${format}:${a}x${b}`,
      prompt: `${a} × ${b} = ?`,
      answer: String(ans)
  ...
  function genDivision(format) {
    ...
    if (format === "B") {
      // Missing divisor or missing dividend? Spec says missing number format is fine:
      // 144 ÷ ? = 12 (missing divisor)
      return {
        id: `D:B:${dividend}/?=${b}`,
        prompt: `${dividend} ÷ ? = ${b}`,
        answer: String(a)
    ...
    // Format C (inverse check): dividend ÷ divisor = ?
    // Format A: same, direct
    return {
  ...
  function genRoots(format) {
    ...
    if (format === "B") {
      // Missing inside-root not needed; keep to integer answer:
      // √? = n (so ? = n^2)
      return {
        id: `R:B:sqrt?=${n}`,
        prompt: `√? = ${n}`,
  ...
  function getTopicMixForMastery(topicId) {
    // If user selects a specific topic mastery, keep to that topic.
    // (Later we can add "Mixed Pillar Mastery".)
    return [topicId];
  }
